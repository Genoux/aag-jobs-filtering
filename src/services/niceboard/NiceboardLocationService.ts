...
export class NiceboardLocationService {
  ...
  async getOrCreateLocation(
    ...
    if (cachedId) {
      console.log(
        `Using cached location ID for "${locationString}": ${cachedId}`,
      )
      return cachedId
    }
    ...
    try {
      ...
      if (existingLocation) {
        ...
        console.log(
          `Found existing location "${locationString}" with ID: ${locationId}`,
        )
        return locationId
      }
      ...
      if (!response.data.error) {
        ...
        this.locationCache.set(locationString, locationId)
        console.log(
          `Created new location "${locationString}" with ID: ${locationId}`,
        )
        return locationId
      }

      console.warn(
        `Failed to create location "${locationString}": ${response.data.message || 'Unknown error'}`,
      )
      return undefined
    } catch (error) {
