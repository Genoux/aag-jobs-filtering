...
import { FetchOptions, UploadOptions, CommandResult } from './types/CLI';
...
class JobProcessor {
  ...
  async fetchJobs(options: FetchOptions): Promise<CommandResult> {
    try {
      ...
      const jobResults = await this.searchService.processAllQueries(options.limit);
  ...
  async uploadJobs(file: string, options: UploadOptions): Promise<CommandResult> {
    try {
...
function main() {
  ...
  program
    ...
    .action((options) => {
      handleCommand(processor.fetchJobs({
  ...
  program
    ...
    .action((file, options) => {
      handleCommand(processor.uploadJobs(file, {
